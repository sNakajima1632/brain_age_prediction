...
class BrainAgeDataset(Dataset):
    def __init__(self, csv_path, root_dir="/home/blue/Blue_Project", target_size=(160, 192, 160)):
        ...
        print(f"  - T1 samples: {(self.df['Modality'] == 'T1').sum()}")
        print(f"  - T2 samples: {(self.df['Modality'] == 'T2').sum()}")
...
# 3. Load Data
# -----------------------
ROOT = "/home/blue/Blue_Project/T1_T2"
...
last_checkpoint_path = f"{ROOT}/last_model.pt"
...
print(f"Training on T1 + T2 combined dataset")
print(f"{'='*60}\n")
...
with open(f"{ROOT}/test_result.txt", 'w') as f:
    f.write(f"Dataset: IXI T1 + T2 Combined\n")
    ...
    f.write(f"\nDataset split:\n")
    f.write(f"Train samples: {len(train_ds)} ({len(train_ds)//2} T1+T2 pairs)\n")
    f.write(f"Val samples: {len(val_ds)} ({len(val_ds)//2} T1+T2 pairs)\n")
    f.write(f"Test samples: {len(test_ds)} ({len(test_ds)//2} T1+T2 pairs)\n")
