...
class ANTSCoregistrationPipeline:
    ...
    def __init__(self,csv_path, template_path, output_dir, t1_path, t2_path,
        ...
        self.t1_path = Path(t1_path) if t1_path is not None else None
        self.t2_path = Path(t2_path) if t2_path is not None else None
    ...
    def register_subject(self, row):
        ...
        -----------
        row : pd.Series
            A row from the dataframe with keys [IXI_ID, T1, T2]
        """
        ...
        try:
            ...
            if temp_subject_dir.exists():
                shutil.rmtree(temp_subject_dir, ignore_errors=True)
            self.log_file = None

    def update_csv(self, dataframe, col_name=None):
        """
        Update CSV file with T1 and T2 image paths based on subject ID.

        Parameters:
        -----------
        dataframe: pandas.DataFrame
            Extracted CSV/Excel file data.
        col_name: str, default=None
            Name of the column to update ("T1" or "T2").

        Raises:
        -------
        ValueError:
            If subject ID column is missing or col_name is invalid.
        """
        # Ensure IXI_ID column exists
        if 'IXI_ID' not in dataframe.columns:
            raise ValueError("Missing column: IXI_ID in CSV")
        
        # Validate column name
        if col_name not in ['T1', 'T2']:
            raise ValueError(f"Unknown column name: {col_name}")
        
        # Add T1/T2 columns if not present
        if col_name not in dataframe.columns:
            dataframe[col_name] = ''

        # Choose directory based on col_name
        img_dir = None
        if col_name == 'T1' and self.t1_path is not None:
            img_dir = self.t1_path
        elif col_name == 'T2' and self.t2_path is not None:
            img_dir = self.t2_path

        if img_dir is None or not Path(img_dir).exists():
            print(f"Skipping update for {col_name} (no directory found).")
            return
    
        # Build regex to match Subject ID
        ...
        for img in os.listdir(img_dir):
            ...
            if match:
                ixi_id = int(match.group(1))
                file_map[ixi_id] = str(Path(img_dir) / img)

        # Keep missing files
        missing_subjects = []

        # Iterate through rows and write paths
        for idx, row in dataframe.iterrows():
            ...
            # Only fill in if empty, and list subjects with missing file
            if not dataframe.at[idx, col_name]:
                if ixi_id in file_map:
                    dataframe.at[idx, col_name] = file_map[ixi_id]
                else:
                    missing_subjects.append(ixi_id)

        if missing_subjects:
            dataframe = dataframe[~dataframe['IXI_ID'].isin(missing_subjects)]
            print(f"Removed {len(missing_subjects)} rows with missing {col_name} images: {missing_subjects}")

        # Save output and update self.csv_path to updated csv
        base, ext = os.path.splitext(self.csv_path)
        if base.endswith("_updated"):
            updated_csv_path = Path(f"{base}{ext}")
        else:
            updated_csv_path = Path(f"{base}_updated{ext}")

        if ext.lower() in ['.xls', '.xlsx']:
            dataframe.to_excel(updated_csv_path, index=False)
        else:
            dataframe.to_csv(updated_csv_path, index=False)
        
        print(f"Updated file saved to: {updated_csv_path}")
        self.csv_path = updated_csv_path
    ...
    def run(self, start_row=0, end_row=None):
        ...
        """
        # Check if updated csv file exists
        base, ext = os.path.splitext(self.csv_path)
        updated_csv_path = Path(f"{base}_updated{ext}")
        if Path(updated_csv_path).exists():
            self.csv_path = updated_csv_path
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Failed to read CSV/Excel file: {e}")

        # Determine if T1 and T2 columns are needed
        required_cols = ['IXI_ID']
        if self.t1_path is not None:
            # Add T1 column if missing
            self.df = self.update_csv(self.df, 'T1')
        else:
            required_cols.append('T1')
        
        if self.t2_path is not None:
            # Add T2 column if missing
...
if __name__ == "__main__":
    ...
    parser.add_argument("--T1_dir", type=str, help="Path to T1 images directory")
    parser.add_argument("--T2_dir", type=str, help="Path to T2 images directory")
